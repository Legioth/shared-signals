...
import { ListSignal } from "../lib/SharedSignals";
...
const cursorCount = computed(() => cursors.value.length);
...
export default function Cursors() {
    ...
    function move(event: MouseEvent): void {
        ...
        if (currentKey.value) {
            cursors.set(currentKey.value, position);
        } else {
            currentKey.value = cursors.insertLast(position).key;
        }
    }

    function leave(event: MouseEvent): void {
        if (currentKey.value) {
            cursors.remove(currentKey.value);
            currentKey.value = "";
        }
    }
    ...
    return (
        <VerticalLayout>
            <div style={{ position: "relative", width: 400, height: 400, border: "1px solid black" }}
                ...
                {
                    cursors.value
                        .filter((signal) => signal.key != currentKey.value)
                        .map((signal) => <Cursor key={signal.key} position={signal} />)
                }
            </div>
            <Button onClick={() => cursors.keys.forEach(key => cursors.remove(key))}>Clear</Button>
            Number of cursors: {cursorCount}
        </VerticalLayout>
    )
}
const Cursor = memo(function Cursor(props: {position: Signal<Position>}) {
    const [left, top] = props.position.value;
